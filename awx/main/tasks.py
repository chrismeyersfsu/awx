...
from awx.main.models import (
    ...
    build_safe_env, migrate_events_to_partitions
)
        ...
        with connection.cursor() as cursor:
            ...
            while total_rows:
                with transaction.atomic():
                    ...
                    cursor.execute(
                        f'DELETE FROM _unpartitioned_{tblname} WHERE id IN (SELECT id FROM _unpartitioned_{tblname} ORDER BY id DESC LIMIT {chunk});'
                    )
                logger.warn(
                    f'migrated rows to partitioned {tblname} from _unpartitioned_{tblname}; # ({last_insert_pk} rows remaining)'
                )
