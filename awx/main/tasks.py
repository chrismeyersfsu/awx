...
class BaseTask(object):
    ...
    def add_ansible_venv(self, venv_path, env, isolated=False):
        ...
        if isolated and not os.path.exists(venv_libdir):
            raise RuntimeError(
        ...
        try:
            ...
            if instance.is_isolated() is True:
                ...
                params = {
                    ...
                    'artifact_dir': 'artifact_dir_explicit',
                    'playbook': self.build_playbook_path(instance, private_data_dir),
                ...
                if self.should_use_proot(instance):
                    process_isolation_params = {
                        ...
                        'process_isolation_show_paths': self.proot_show_paths,
...
class RunJob(BaseTask):
    ...
    def build_env(self, job, private_data_dir, isolated=False, private_data_files=None):
        ...
        if private_data_files is None:
            private_data_files = {}
        env = self.add_ansible_venv(job.ansible_virtualenv_path, env, isolated=isolated)
        # Set environment variables needed for inventory and job event
        # callbacks to work.
    ...
    def build_playbook_path(self, job, private_data_dir):
        cwd = self.build_cwd(job, private_data_dir)
        return os.path.join('../../../../../../../', cwd, job.playbook)
...
class RunProjectUpdate(BaseTask):
    ...
    def build_playbook_path(self, project_update, private_data_dir):
        cwd = self.build_cwd(project_update, private_data_dir)
        return os.path.join('../../../../../../../', cwd, 'project_update.yml')
...
class RunInventoryUpdate(BaseTask):
    ...
    def build_playbook_path(self, inventory_update, private_data_dir):
        return None
