...
from awx.main.utils import get_event_partition_epoch
...
def partition_suffix(d, minutely=False):
    if not minutely:
        return d.replace(microsecond=0, second=0, minute=0).strftime('%Y%m%d_%H')
    else:
        return d.replace(microsecond=0, second=0).strftime('%Y%m%d_%H')
...
class DeleteMeta():
    # TODO: Maybe 1 big transaction ? Look into how delete worked before
    ...
    def find_jobs_to_delete(self):
        ...
        self.jobs_no_delete_count = (
            self.job_class.objects.filter(created__gte=self.cutoff) |
            self.job_class.objects.filter(status__in=['pending', 'waiting', 'running'])
        ).count()
    ...
    def drop_partitions(self):
        if self.parts_to_drop and not self.dry_run:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    f"DROP TABLE {parts_to_drop_str}"
                )
