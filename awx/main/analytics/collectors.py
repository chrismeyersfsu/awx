...
def events_slicing_partitioned(key, since, until):
    # TODO: Refactor cleanup_jobs.py, where we took the below partition logic from
    tbl_name = 'main_jobevent'
    lower, last_entries = _identify_lower(key, since, until)
    ...
    with connection.cursor() as cursor:
        ...
        query += f" AND TO_TIMESTAMP(LTRIM(inhrelid::regclass::text, '{tbl_name}_'), 'YYYYMMDD_HH24') > '{lower}' and TO_TIMESTAMP(LTRIM(inhrelid::regclass::text, '{tbl_name}_'), 'YYYYMMDD_HH24') <= '{until}'"
        query += " ORDER BY inhrelid::regclass::text"

        cursor.execute(query)
        partitions_from_db = [r[0] for r in cursor.fetchall()]

    for partition_name in partitions_from_db:
        print(f"Acting on partition {partition_name}")
        yield (partition_name, None)
...
# The register decorator is intentionally commented out
# We found that per-partition event analytics gathering is much more performant than using per-partition index
